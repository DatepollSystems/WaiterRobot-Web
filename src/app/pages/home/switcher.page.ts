...
@Component({
  template: `
    <form [formGroup]="form" (ngSubmit)="select()">
      ...
      <div class="d-flex justify-content-between gap-4 pb-5">
        @if (!modal()) {
          <h1 class="fw-bold mb-0 fs-2">Wähle Organisation und Event aus</h1>
        }

        <div>
          <button class="btn btn-lg btn-primary" [disabled]="form.invalid" type="submit">Auswählen</button>
        </div>
      </div>

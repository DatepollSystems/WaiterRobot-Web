  ...
  intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    if (!toIntercept) {
      ...
      if (this.authService.isJWTTokenValid()) {
        req = OldAuthInterceptor.addToken(req, this.authService.getJWTToken());
      }
    ...
    if (!this.isRefreshing) {
      ...
      return this.authService.refreshJWTToken().pipe(
        switchMap((data: JWTResponse) => {
          ...
          return next.handle(OldAuthInterceptor.addToken(request, data.token));
        }),
        catchError(() => {
      ...
      return this.refreshTokenSubject.pipe(
        ...
        switchMap((jwt: string) => {
          ...
          return next.handle(OldAuthInterceptor.addToken(request, jwt));
        })
      );
    }
  }
